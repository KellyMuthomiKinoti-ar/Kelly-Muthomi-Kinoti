  ...
  return (
    <section id="projects" aria-labelledby="projects-heading" className="py-24 bg-slate-950 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Compact Engineering Statistics Ribbon — Low profile on mobile & tablet */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-6">
          <div className="bg-slate-900/80 border border-slate-800/80 px-3 py-2.5 rounded-xl flex items-center justify-between">
            <div>
              <div className="text-xs text-slate-400 uppercase font-mono tracking-wider">Deployments</div>
              <div className="text-base sm:text-lg font-bold font-mono text-white">4 Production</div>
            </div>
            <Server className="w-4 h-4 text-amber-400/60 shrink-0" />
          </div>
          <div className="bg-slate-900/80 border border-slate-800/80 px-3 py-2.5 rounded-xl flex items-center justify-between">
            <div>
              <div className="text-xs text-slate-400 uppercase font-mono tracking-wider">Uptime SLA</div>
              <div className="text-base sm:text-lg font-bold font-mono text-amber-400">99.9%</div>
            </div>
            <Shield className="w-4 h-4 text-amber-400/60 shrink-0" />
          </div>
          <div className="bg-slate-900/80 border border-slate-800/80 px-3 py-2.5 rounded-xl flex items-center justify-between">
            <div>
              <div className="text-xs text-slate-400 uppercase font-mono tracking-wider">Mobile LCP</div>
              <div className="text-base sm:text-lg font-bold font-mono text-emerald-400">&lt;0.9s</div>
            </div>
            <Zap className="w-4 h-4 text-emerald-400/60 shrink-0" />
          </div>
          <div className="bg-slate-900/80 border border-slate-800/80 px-3 py-2.5 rounded-xl flex items-center justify-between">
            <div>
              <div className="text-xs text-slate-400 uppercase font-mono tracking-wider">Type Safety</div>
              <div className="text-base sm:text-lg font-bold font-mono text-cyan-400">100%</div>
            </div>
            <CheckCircle2 className="w-4 h-4 text-cyan-400/60 shrink-0" />
          </div>
        </div>

        {/* Direct Scroll Anchor for Featured Projects Grid */}
        <div id="featured-projects" className="scroll-mt-28" />
        <div id="projects-grid" className="scroll-mt-28" />

        {/* Compact Search, Filter & Sort Bar */}
        <div className="bg-slate-900/70 border border-slate-800/80 rounded-2xl p-3 sm:p-4 mb-8 space-y-3">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                className="w-full pl-10 pr-9 py-2 rounded-xl bg-slate-950 border border-slate-700/80 text-slate-200 text-xs sm:text-sm focus:outline-none focus:border-amber-500 transition-colors"
              />
              {searchQuery && (
                <button
            ...
            <div className="flex items-center gap-2 self-start lg:self-auto">
              ...
              <select
                ...
                className="bg-slate-950 border border-slate-700 text-slate-200 text-xs font-mono rounded-xl px-3 py-1.5 focus:outline-none focus:border-amber-500"
              >
          ...
          {/* Category & Tech Pills — Responsive single/two-line flow */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pt-2 border-t border-slate-800/80">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              <span className="text-[11px] font-mono text-slate-400 mr-1 hidden sm:inline">Tech:</span>
              {techFilters.map((tf) => {
        ...
        {/* Projects Grid — Beautifully Organized Cards with zero overflow */}
          ...
          {filteredProjects.map((project) => {
            ...
            return (
              <Card
                key={project.id}
                variant="interactive"
                className="p-5 sm:p-7 flex flex-col justify-between group overflow-hidden"
              >
                <div>
                  {/* Top Status & Verification Badge Bar — Never Overflows */}
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
                    ...
                    <div className="text-[10px] font-mono text-emerald-400 bg-emerald-950/70 border border-emerald-500/30 px-2.5 py-0.5 rounded-full flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                      <span>VERIFIED PROD SLA</span>
                    </div>
                  </div>

                  {/* Sleek Browser Address Bar & Architectural Stack Preview */}
                  <div className="w-full rounded-xl bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 border border-slate-800/90 mb-6 p-3.5 sm:p-4 flex flex-col justify-between relative overflow-hidden group-hover:border-amber-500/40 transition-colors">
                    
                    {/* Simulated Browser Bar with Visit Site CTA — Zero Overflow */}
                    <div className="flex items-center justify-between gap-2 bg-slate-950/95 border border-slate-800 rounded-lg px-3 py-2 mb-3.5 shadow-inner">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-[10px] font-mono font-bold text-amber-400 hover:text-amber-300 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 px-2 py-0.5 rounded transition-colors flex items-center gap-1 shrink-0"
                        title={`Visit live site: ${project.displayUrl}`}
                    ...
                    <div className="flex items-center justify-between relative z-10">
                      <div className="min-w-0 pr-2">
                        <div className="text-[10px] font-mono text-slate-400 uppercase tracking-widest">Architecture Stack</div>
                        <div className="text-xs sm:text-sm font-bold text-white font-mono truncate">
                          {project.techStack.slice(0, 4).join(' • ')}
                        </div>
                      </div>
                      <div className="p-2 rounded-xl bg-slate-900/90 border border-slate-700/80 text-amber-400 shadow-md shrink-0">
                        {getCategoryIcon(project.category)}
                      </div>
                    </div>
                  ...
                  {project.challengesSolved && project.challengesSolved.length > 0 && (
                    <div className="bg-amber-950/20 border border-amber-500/30 p-3.5 sm:p-4 rounded-xl mb-5 space-y-1.5">
                ...
                {/* Actions Footer — Clean mobile/tablet stacking, zero button overflow */}
                <div className="pt-4 border-t border-slate-800/80 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
                  <Button
                    ...
                    className="w-full sm:w-auto flex-1 justify-center py-2.5"
                    aria-label={`View Case Study for ${project.title}`}
                  >
                    <Layers className="w-3.5 h-3.5 mr-1.5 shrink-0" />
                    <span>Case Study &amp; Architecture Specs</span>
                  </Button>
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    <Button
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex-1 sm:flex-initial justify-center py-2.5 bg-slate-900/80 hover:bg-amber-500/10 border-slate-700 hover:border-amber-500/40 text-amber-400"
                      aria-label={`Open Live Demo for ${project.title}`}
                    >
                      <Globe className="w-3.5 h-3.5 mr-1.5 shrink-0 text-amber-400" />
                      <span>Live Site</span>
